...
struct VisionUpdateBundle {
   ...
   VisionUpdateBundle(
         ...
         const std::vector<SharedBallInfo> &compactSharedBalls,
         ...
         const bool isDistanceReliable) :
            ...
            compactSharedBalls(compactSharedBalls),
   ...
   // Shared balls. We share balls between robots for use in heat-map
...
inline std::ostream& operator<<(std::ostream& os, const VisionUpdateBundle& bundle) {
   ...
   unsigned numSharedBalls = bundle.compactSharedBalls.size();
   os.write((char*) &numSharedBalls, sizeof(unsigned));
   for (unsigned i=0; i<bundle.compactSharedBalls.size(); i++)
   {
      os << bundle.compactSharedBalls[i];
   }
...
inline std::istream& operator>>(std::istream& is, VisionUpdateBundle& bundle) {
   ...
   unsigned numSharedBalls;
   is.read((char*) &numSharedBalls, sizeof(unsigned));
   bundle.compactSharedBalls.clear();
   for (unsigned i = 0; i < numSharedBalls; i++) {
      SharedBallInfo newBall;
      is >> newBall;
      bundle.compactSharedBalls.push_back(newBall);
   }   

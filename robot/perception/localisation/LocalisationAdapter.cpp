...
void LocalisationAdapter::tick() {
   ...
   std::vector<BallInfo> sharedBalls = readFrom(vision, sharedBalls);
   std::vector<SharedBallInfo> compactSharedBalls;

   AbsCoord currentRobotPos = readFrom(localisation, robotPos);
   for (unsigned i=0; i < sharedBalls.size(); i++)
   {
      compactSharedBalls.push_back(SharedBallInfo(sharedBalls[i], currentRobotPos));
   }
...
//   }
   ...
   // TODO (Peter): Should probably do this later in writeResultToBlackboard
   // but I don't understand why these decisions were made and
   // no one seems to have documented them so happy to relearn mistakes
   double heading = getRobotHeadingSuggestion(visionUpdateBundle);
   writeTo(localisation, robotHeadingSuggestion, heading)
...
double LocalisationAdapter::getRobotHeadingSuggestion(VisionUpdateBundle visionUpdateBundle) {
   double heading = 0;
   std::vector<FieldFeatureInfo> closestLines;
   float closestDistance = 1000000.0;  // Would prefer MAX_FLOAT
   for (unsigned i = 0; i < visionUpdateBundle.fieldFeatures.size(); i++) {
      if (visionUpdateBundle.fieldFeatures[i].type == FieldFeatureInfo::fLine) {
         // Note this will throw away lines
         if (visionUpdateBundle.fieldFeatures[i].distance() < closestDistance) {
             closestLines.push_back(visionUpdateBundle.fieldFeatures[i]);
         }
      }
   }
   if (closestLines.size() > 0) {
      FieldFeatureInfo closestLine = closestLines[closestLines.size() - 1];
      // Try going to the furthest away point of the closest line
      double p1DistanceSquared = pow(closestLine.line.p1.x(), 2) + pow(closestLine.line.p1.y(), 2);
      double p2DistanceSquared = pow(closestLine.line.p2.x(), 2) + pow(closestLine.line.p2.y(), 2);
      if (p1DistanceSquared > p2DistanceSquared) {
         heading = atan2(closestLine.line.p1.y(), closestLine.line.p1.x());
//         std::cout << "Choose P1(" << closestLine.line.p1.x() << "," <<
//         closestLine.line.p1.y() << "," << RAD2DEG(heading) << ") " <<
//         " @ distance " << closestLine.distance() << std::endl;
      } else {
         heading = atan2(closestLine.line.p2.y(), closestLine.line.p2.x());
//         std::cout << "Choose P2(" << closestLine.line.p2.x() << "," <<
//         closestLine.line.p2.y() << "," << RAD2DEG(heading) << ") " <<
//         " @ distance " << closestLine.distance() << std::endl;
      }
   }
   return heading;
...
void LocalisationAdapter::writeResultToBlackboard(void) {
   ...
   std::vector<BallInfo> receivedBalls = L->getReceivedBalls();
   ...
   writeTo(localisation, receivedBalls, receivedBalls);

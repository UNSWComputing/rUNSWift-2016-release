...
void Localiser::localise(const LocaliserBundle &lb, const bool canDoObservations) {
   ...
   if (lb.visionUpdateBundle.compactSharedBalls.size() > 0)
   {
      sharedDistribution->sharedBallsUpdate(lb.visionUpdateBundle.compactSharedBalls);
   }
...
std::vector<BallInfo> Localiser::getReceivedBalls()
{
   std::vector<BallInfo> recv;
   AbsCoord robotPos = getRobotPose();

   for (std::map<int, std::vector<SharedBallInfo> >::iterator m = receivedBallMap.begin(); m != receivedBallMap.end(); m++)
   {
      for (std::vector<SharedBallInfo>::iterator itr = m->second.begin(); itr != m->second.end(); itr++)
      {
         SharedBallInfo shared = *itr;
         BallInfo b = shared.toBallInfo(robotPos);
         recv.push_back(b);
      }
      receivedBallMap.erase(m);
   }
   return recv;
...
void Localiser::applyRemoteUpdate(const BroadcastData &broadcastData, int playerNumber) {
   ...
   receivedBallMap[playerNumber] = std::vector<SharedBallInfo>();
   for (int i = 0; i < MAX_SHARED_BALLS; i++)
   {
      if (broadcastData.sharedLocalisationBundle.sharedBalls[i].visionVar > 0) 
         receivedBallMap[playerNumber].push_back(broadcastData.sharedLocalisationBundle.sharedBalls[i]);
   }
}

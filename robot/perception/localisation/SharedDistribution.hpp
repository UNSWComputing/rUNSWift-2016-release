...
public:
   ...
   void sharedBallUpdate(const BallInfo &sharedBall);

   void visionUpdate(const StoredICPUpdate &icpUpdate, const UniModalVisionUpdate &vu);

   void sharedBallsUpdate(const std::vector<SharedBallInfo> sharedBalls);
...
private:
   ...
   std::vector<SharedBallInfo> sharedBalls;
};

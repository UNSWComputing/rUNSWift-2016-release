...
template <hist_mask_t hmask, edge_weights_t edge_weights>
void FoveaT<hmask, edge_weights>::actuateWithoutColour(const VisionFrame &frame)
{
   makeFoveaWithoutColour(frame);
   if (edge_weights) {
      blurGrey();
      makeEdge();
   }
...
// Same as above, but without colour
// j is the number of columns
// i is the number of rows
template <hist_mask_t hmask, edge_weights_t edge_weights>
void FoveaT<hmask, edge_weights>::makeFoveaWithoutColour (const VisionFrame &frame)
{
   const uint16_t COLS = (top) ? TOP_IMAGE_COLS : BOT_IMAGE_COLS;
...
template <hist_mask_t hmask, edge_weights_t edge_weights>
const int &FoveaT<hmask, edge_weights>::magnitude(int x, int y) const
{
   return (_edge[x * bb.height() + y].x()*_edge[x * bb.height() + y].x())
         + (_edge[x * bb.height() + y].y()*_edge[x * bb.height() + y].y());
}
...
const int Fovea::magnitude(int x, int y) const
{
   return (edge(x, y)[0]*edge(x, y)[0])
...
template <hist_mask_t hmask, edge_weights_t edge_weights>
void FoveaT<hmask, edge_weights>::zeroEdge(int x, int y)
{
   edge(x,y)[0] = 0;
   edge(x,y)[1] = 0;
}

// Binary erosion algorithm
// If anything above the weak threshold is touching a weak threshold pixel
// it is zeroed. Harsh, perhaps, but this is an initial test
// This is for removing extra edges in ball detection
// NOTE: For now, this actually modifies the edge image.

// NOTE: we get a potential speedup by ignoring the edges, but
// we also miss some points. Worth the tradeoff? I doubt it

#define WEAK_EDGE (200*100)
#define STRONG_EDGE (200*200) // tweak?
#define MAG(A,B) (A*A + B*B)
#define ID (2)

/* Erode edges. Modifies the edge saliency */
template <hist_mask_t hmask, edge_weights_t edge_weights>
void FoveaT<hmask, edge_weights>::erode()
{
   int x, y;
   int dx, dy;
   ...
   for (y = 0; y < bb.height(); ++ y) {
      for (x = 0; x < bb.width(); ++ x) {
         dx = edge(x,y)[0];
         dy = edge(x,y)[1];
         if (dy != ID && MAG(dx,dy) < WEAK_EDGE) {
            // Check all surrounding edges that are unvisited and large
            if (x != 0) {
               dx = edge(x-1,y)[0];
               dy = edge(x-1,y)[1];
               // Too small, erode
...
/* Dilate edges. Modifies the edge saliency */
template <hist_mask_t hmask, edge_weights_t edge_weights>
void FoveaT<hmask, edge_weights>::dilate()
{
   int x, y;
   int dx, dy;
   ...
   for (y = 1; y < bb.height()-1; ++ y) {
      for (x = 1; x < bb.width()-1; ++ x) {
         dx = edge(x,y)[0];
         dy = edge(x,y)[1];
         if (dy != ID && MAG(dx,dy) > STRONG_EDGE) {
            // Check all surrounding edges that are unvisited and large
            dx = edge(x-1,y)[0];
            dy = edge(x-1,y)[1];
            // Too small, erode
            if (dy != ID && MAG(dx,dy) <= STRONG_EDGE) {
               edge(x-1,y)[0] = STRONG_EDGE+1;
               edge(x-1,y)[1] = ID;
            }

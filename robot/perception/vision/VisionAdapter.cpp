...
VisionAdapter::VisionAdapter(Blackboard *bb)
   : Adapter(bb),
     V((blackboard->config)["vision.dumpframes"].as<bool>(),
       ...
       (blackboard->config)["player.number"].as<int>(),
...
void VisionAdapter::tick() {
   ...
   V.getCameraSettings();
   writeTo(vision, topCameraSettings, V.topCameraSettings);
   writeTo(vision, botCameraSettings, V.botCameraSettings);
   ...
   V.ballDetection.ballVelRR = readFrom(localisation, ballVelRRC).convertToRobotRelative();
   V.ballDetection.latestAngleX = values.sensors[Sensors::InertialSensor_AngleX];
   V.ballDetection.localisationRobots = readFrom(localisation, robotObstacles);

   /* Read shared balls from other robots */
   V.receivedBalls = readFrom(localisation, receivedBalls);
   ...
   writeTo (vision, sharedBalls,    V.sharedBalls  );

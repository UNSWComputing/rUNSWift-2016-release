...
void RobotDetection::setDetectedRobots(
    ...
    for (unsigned int botIndex = 0; botIndex < botPossibleRobots.size(); ++botIndex) {
        ...
        RobotInfo robot(botRobot.feet, RobotInfo::rUnknown, region, RobotInfo::BOT_CAMERA);
        ...
        if (matchedTop != -1) {
            ...
            robot.cameras = RobotInfo::BOTH_CAMERAS;
            ...
            if (topRobot.type == PossibleRobot::RED_TEAM) {
                ...
                type = RobotInfo::rBlue;
            } else {
                type = RobotInfo::rUnknown;
            }
    ...
    for (unsigned int topIndex = 0; topIndex < topPossibleRobots.size(); ++topIndex) {
        if (useTop[topIndex]) {
           ...
           RobotInfo robot(r.feet, type, region, RobotInfo::TOP_CAMERA);

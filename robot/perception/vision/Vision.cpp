...
void Vision::getCameraSettings() {
    NaoCamera* top = (NaoCamera*)(top_camera);
    NaoCamera* bot = (NaoCamera*)(bot_camera);
    topCameraSettings = top->topCameraSettings;
    botCameraSettings = bot->botCameraSettings;
}
...
void Vision::processFrame()
{
   ...
   frame = boost::shared_ptr<VisionFrame>(
         new VisionFrame(topFrame, topNnmc, botFrame, botNnmc, convRR, frame, receivedBalls));
   ...
   // Save the detected robots to the frame list for use by ball detection
   //frame->robots.insert(frame->robots.end(), robots.begin(), robots.end());
   ballDetection.robots.insert(ballDetection.robots.end(), robots.begin(), robots.end());
   //ballDetection.topPossRobots.insert(ballDetection.topPossRobots.end(), robotDetection._topPossibleRobots.begin(), robotDetection._topPossibleRobots.end());
   ballDetection.botPossRobots.insert(ballDetection.botPossRobots.end(), robotDetection._botPossibleRobots.begin(), robotDetection._botPossibleRobots.end());
...
*/
   ...
   /*******************************************************************
    * Ball Detection - needs Robot Detection, Field Feature Detection *
    *******************************************************************/
   timer.restart();

   // Get the points from lines in Field Feature Detection
   // Give them to ball detection
   ballDetection.lineEdges = fieldLineDetection.lineEdges;

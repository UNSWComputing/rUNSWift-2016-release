...
std::vector<BBox> &regions,bool secondCheck) {
    ...
    for (it = candidates.begin(); it != candidates.end(); ++it) {
        ...
        for(int  row = it->a.y();row < it->b.y();row++){
        	if (row >= fovea.bb.height()){
        		continue;
        	}
        	if(it->a.x() < 0){
        		continue;
        	}
        	if(it->b.x() >= fovea.bb.width()){
        		continue;
        	}
        	int column = it->a.x();
        	Point edge = fovea.edge(column, row);
        	float magnitudex = edge.x();
        	if(abs(magnitudex) > 100){
        	  leftEdge++;
        	}
        }
        ...
        if (float (numColourPixels / (int)(length/4)) < 0.8) {
        ...
        for (int row = it->b.y()-(length/4); row != it->b.y()-length; --row) {
        	Point edge = fovea.edge(centre, row);
        	float magnitudey = edge.y();
        	   if(magnitudey < -150 || magnitudey > 150){
        	      numEdges++;
        	   }
        ...
        //Post must not contain too many strong horizontal edges traversing vertically
        if(numEdges > 12){
        	//cout << "Throwing out with " << numEdges <<" of " <<length <<endl;
        	continue;
        }
        ...
        if(leftEdge/length < 0.75 && rightEdge/length < 0.75){
            continue;
        }
        regions.push_back(*it);
    }
...
bool update) {
    ...
    br.x() = std::min(br.x() + width/2, TOP_IMAGE_COLS-1);

    if (tl.x() >= br.x()) return -1;
    if (tl.y() >= br.y()) return -1;
    if (tl.x() >= TOP_IMAGE_COLS || br.x() >= TOP_IMAGE_COLS) return -1;
    if (tl.y() >= TOP_IMAGE_ROWS || br.y() >= TOP_IMAGE_ROWS) return -1;
    if (tl.x() < 0 || br.x() < 0) return -1;
    if (tl.y() < 0 || br.y() < 0) return -1;

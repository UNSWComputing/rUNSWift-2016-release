...
#define KICK_STEP_HEIGHT 0.055 // how far to lift kicking foot
#define BACK_PHASE 0.75 // time to move kick foot back
#define MIN_KICK_PHASE 0.3 // time to swing kick foot
...
#define KICK_LEAN_V4 22.1
#define KICK_LEAN_V5 21.7
...
JointValues Walk2014Generator::makeJoints(ActionCommand::All* request, Odometry* odometry, const SensorValues &sensors, BodyModel &bodyModel, float ballX, float ballY) {
    ...
    if (t == 0 && walk2014Option != STEP) {
        ...
        speed = active.speed;                     // controls speed of walk
...
void Walk2014Generator::walkKick(float ballX, BodyModel &bodyModel) {
    ...
    if (ActionCommand::Body::LEFT) { // left foot walk-kick
        ...
        if (bodyModel.isLeftPhase and (ballDistX - delta + forwardL0) < delta)
            forward = abs(power) * MAX_FORWARD;
        if (!bodyModel.isLeftPhase and (ballDistX - delta - forwardR0) < delta)
            forward = abs(power) * MAX_FORWARD;
    ...
    if (forward > MAX_FORWARD)
        forward = MAX_FORWARD; // in case things go wrong
    if (forward > MAX_FORWARD)
        forward = MAX_FORWARD; // in case things go wrong
...
bool Walk2014Generator::canAbbortKick() {
    ...
    return kickT - pauseTime < halfShift || (kickT >= totalShift + pauseTime && kickT < BACK_PHASE);
}

void Walk2014Generator::makeForwardKickJoints(float kickLean, float kickStepH, float shiftPeriod, float &footh, float &forwardDist, float &side, float &kneePitch,
    ...
    if (kickT < BACK_PHASE) {
        ...
        if (kickT >= BACK_PHASE) {
            anklePitch = interpolateSmooth(anklePitchRetracted, anklePitchAmp, kickT - BACK_PHASE, kickPhase);
        }
        // Hold...

...
// If we are player 1, then use the old getup routines. Else, the new ones
void DistributedGenerator::setPlayerSpecificGetups(int playerNum) {
   if (playerNum != 1) {
      // Change the getup routines to the newer faster ones
      GetupGenerator * gFront = new GetupGenerator("UChileGetUpFront");
      gFront->setPlayerSpecificGetup(playerNum);
      GetupGenerator * gBack = new GetupGenerator("HTWKGetupBack");
      gBack->setPlayerSpecificGetup(playerNum);

      bodyGenerators[Body::GETUP_FRONT] = (Generator*) gFront;
                                       //(new GetupGenerator("UChileGetUpFront"));
      if (!bodyGenerators[Body::GETUP_FRONT])
         llog(FATAL) << "bodyGenerators[GETUP_FRONT] is NULL!" << std::endl;

      bodyGenerators[Body::GETUP_BACK] = (Generator*) gBack;
                                      //(new GetupGenerator("HTWKGetupBack"));
      if (!bodyGenerators[Body::GETUP_BACK])
         llog(FATAL) << "bodyGenerators[GETUP_BACK] is NULL!" << std::endl;
   }
}

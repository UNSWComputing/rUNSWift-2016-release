...
#  3. brew install wget
#  4. brew install cmake  # Should get cmake 3.1.1 or later, errors on 2.8.12.2
#  5. Put ${OSX_CTC_ZIP} into rUNSWift/ctc directory.
#     # You have to get it from Aldebaran at this time.
#  6. bin/build_setup.sh  # Should build libagent and runswift
#  7. nao_sync to the robot as under Linux, e.g. nao_sync -ar loki.local
#  8. offnao does not work with OSX yet, it is not clear
...
cd $RUNSWIFT_CHECKOUT_DIR
mkdir GameController2016
cd GameController2016
wget www.tzi.de/spl/pub/Website/Downloads/GameController2016.zip
unzip -q GameController2016.zip
